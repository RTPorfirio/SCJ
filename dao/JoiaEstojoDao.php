...
class JoiaEstojoDao {
    ...
    public static function EstojoListaJoias() {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $busca = $conn->prepare("select * from joiaestojo");
        $busca->execute();
    ...
    public static function EstojoListaJoia($id_joia) {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $busca = $conn->prepare("select * from joiaestojo where id_joia=:id");
    ...
    public static function EstojolistaJoiaQR($qrCode, $teste) {

        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $busca = $conn->query("SELECT * FROM `joiaestojo` WHERE `qr_code` = '$qrCode'");
        $sm = $busca->fetch(PDO::FETCH_ASSOC);
        $joia = self::CreateJoia($sm);
        return $joia;
    }

    public static function EstojoSelecionaUltimoId() {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $busca = $conn->prepare("SELECT MAX(id_joia) joiaestojo joias");
        $busca->execute();
        $sm = $busca->fetch(PDO::FETCH_ASSOC);
        return $sm;
    }

    public static function EstojoInsereJoia($joia) {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $insere = $conn->prepare("INSERT INTO `joiaestojo` (`preco_custo`, `preco_venda`, `consignado`, `nota_fiscal`, `obs`, `cod_base`, `qr_code`, `tamanho`, `imagem`,`id_tipo`,`id_loja`,`id_cor`,`id_pedra`,`valida`) VALUES (:preco_custo, :preco_venda, :consignado,  :nota_fiscal, :obs, :cod_base, :qr_code,:tamanho, :imagem,:tipo,:loja,:cor,:pedra,:valida)");
    ...
    public static function EstojoeditaValida($joia) {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $edita = $conn->prepare("UPDATE `joiaestojo` SET "
                . "`valida` = :valida "
                . "WHERE `id_joia` = :id;");
    ...
    public static function EstojoDeletaJoia($id_joia) {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $del = $conn->prepare("DELETE FROM `crisjoias`.`joiaestojo` WHERE `joia`.`id_joia` = :id");
    ...
    public static function EstojoEditaJoia($joia) {
        $conexao = new ConnectBD();
        $conn = $conexao->connectBD();
        $edita = $conn->prepare("UPDATE `joiaestojo` SET "
